...
TEST(posix_disk_manager_create) {
    ...
    DiskManager::PosixDiskManager posix_disk_manager{Tests::DiskManagerOpts::temp_storage_dir_path,
                                Tests::DiskManagerOpts::page_byte_size,
                                Tests::DiskManagerOpts::max_disk_byte_size};
}
...
TEST(posix_disk_manager_disk_single_page_read_write) {
    ...
    auto written_bytes = new uint8_t[Tests::DiskManagerOpts::page_byte_size];
    for(int i = 0; i < Tests::DiskManagerOpts::page_byte_size; i++) {
    ...
    auto read_bytes = new uint8_t[Tests::DiskManagerOpts::page_byte_size];
    ...
    ASSERT_EQUAL(posix_disk_manager.disk_byte_size(posix_disk_test_helper.get_disk_id()).get_value(), Tests::DiskManagerOpts::page_byte_size);
...
TEST(posix_disk_manager_disk_multi_page_read_write) {
    ...
    for(int cur_page = 0; cur_page < Tests::DiskManagerOpts::max_disk_num_pages - 1; cur_page++) {
        for(int i = 0; i < Tests::DiskManagerOpts::page_byte_size; i++) {
            written_bytes[i] = i % sizeof(uint8_t);
        }
        ...
        ASSERT_EQUAL(posix_disk_manager.disk_byte_size(posix_disk_test_helper.get_disk_id()).get_value(), Tests::DiskManagerOpts::page_byte_size * (cur_page + 1));
    }
...
TEST(posix_disk_manager_disk_exceeds_max_size) {
    ...
    auto written_bytes = new uint8_t[Tests::DiskManagerOpts::page_byte_size];
    auto write_res = posix_disk_manager.write(posix_disk_test_helper.get_disk_id(), Tests::DiskManagerOpts::max_disk_num_pages, written_bytes);
