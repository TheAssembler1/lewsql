...
void disk_test() {
    ...
    DiskId disk_id = disk_manager->create(TEST_DISK_NAME);
    ASSERT(disk_manager->num_loaded_disks() == 1);

    disk_manager->destroy(disk_id);
    ASSERT(disk_manager->num_loaded_disks() == 0);
    ...
    for(int i = 0; i < NUM_DISKS_TO_TEST; i++) {
        ...
        loaded_disks.push_back(disk_manager->create(disk_name));
    }

    ASSERT(disk_manager->num_loaded_disks() == NUM_DISKS_TO_TEST);
