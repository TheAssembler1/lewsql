...
BufferPageCursor DumbAlg::get_victim(std::vector<bool>& pool_bitmap, std::unique_ptr<uint8_t[]>& mem_pool, unsigned int page_size) {
    ...
    for(auto bit_page: pool_bitmap) {
        if(bit_page) {
            ...
            BufferPage* buffer_page = reinterpret_cast<BufferPage*>(&mem_pool[((sizeof(BufferPage) + page_size) * buffer_page_cursor)]);
